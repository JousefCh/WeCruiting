...
export default function MinimalTemplate({ data, primaryColor, fontFamily, pageCount = 1, backgroundOnly = false }) {
  const { personalInfo: p = {}, workExperience = [], education = [], skills = [], languages = [], hobbies = [] } = data;

  if (backgroundOnly) {
    return (
      <div style={{ fontFamily, width: 794, minHeight: 1123 * pageCount, background: '#fff', position: 'relative' }}>
  ...
  const Section = ({ title, children }) => (
    <div style={{ marginBottom: 24, display: 'flex', gap: 20 }}>
      <div style={{ width: 140, minWidth: 140, paddingTop: 2 }}>
        <span style={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1.5, color: primaryColor }}>{title}</span>
      </div>
  ...
  return (
    <div style={{ fontFamily, width: 794, minHeight: 1123 * pageCount, background: '#fff', fontSize: 16, color: '#1a1a2e', padding: '48px 48px 48px 36px', position: 'relative' }}>
      ...
      <div style={{ paddingLeft: 12 }}>
        ...
        <div style={{ marginBottom: 36 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
            <div>
              <div style={{ fontSize: calcNameFontSize(fullName, 49), fontWeight: 700, letterSpacing: -1, lineHeight: 1.1, color: '#0a0a0a', wordBreak: 'break-word' }}>{fullName}</div>
              {workExperience[0]?.position && (
                <div style={{ fontSize: 20, color: primaryColor, fontWeight: 500, marginTop: 7 }}>{workExperience[0].position}</div>
              )}
            </div>
            {p.photo && (
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 20px', marginTop: 14, fontSize: 15, color: '#6b7280' }}>
          ...
          <Section title="Berufserfahrung">
            {workExperience.map((job, idx) => (
              <div key={job.id} data-cv-item="work" style={{ marginBottom: idx < workExperience.length - 1 ? 15 : 0 }}>
                ...
                <div style={{ fontSize: 16, color: '#6b7280', marginBottom: 4 }}>{job.company}</div>
                {job.description && (
          ...
          <Section title="Ausbildung">
            {education.map((edu, idx) => (
              <div key={edu.id} data-cv-item="edu" style={{ marginBottom: idx < education.length - 1 ? 13 : 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  <div style={{ fontWeight: 700, fontSize: 18 }}>{edu.degree}{edu.field ? ` – ${edu.field}` : ''}</div>
                  <div style={{ fontSize: 14, color: '#9ca3af' }}>
                    {edu.startDate} – {edu.current ? 'heute' : edu.endDate}
                  </div>
                </div>
                <div style={{ fontSize: 16, color: '#6b7280' }}>{edu.institution}</div>
                {edu.grade && <div style={{ fontSize: 15, color: '#9ca3af' }}>Note: {edu.grade}</div>}
              </div>
            ))}
          </Section>
        )}
        ...
        {skills.length > 0 && (
          <Section title="Kenntnisse">
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '7px 22px' }}>
              {skills.map(s => (
                <div key={s.id} style={{ fontSize: 15 }}>
                  <span style={{ color: '#1a1a2e' }}>{s.name}</span>
                  {s.level > 0 && (
                    <span style={{ color: primaryColor, marginLeft: 5, fontSize: 14 }}>
